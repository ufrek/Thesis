...
//added get methods for  min and max values
//modified update text to show correct time format and update clip selection duration
namespace MinMax_Slider
{
	...
	public class MinMaxSlider : Selectable, IBeginDragHandler, IDragHandler, IEndDragHandler
	{
		...
		[SerializeField] private TextMeshProUGUI clipDuration = null;
		...
		private void UpdateText()
		{
			...
			if (minText)
			{
				minutes = (int)minValue / 60;
				seconds = (int)minValue % 60;
				...
				//minText.SetText(minValue.ToString(textFormat));  //in plain seconds format
			}
			...
			if (maxText)
			{
				...
				maxText.SetText(time);
				//maxText.SetText(maxValue.ToString(textFormat)); //in plain seconds format
			}
			float duration = maxValue - minValue;
			clipDuration.text = "Selection Duration: " + (duration).ToString() + " seconds";
			if (duration > 3 && duration < 10)
			{
				clipDuration.color = Color.green;
			}
			else
				clipDuration.color = Color.white;
